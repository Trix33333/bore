...
class Database:
        ...
        with self._conn:
            self._conn.executescript("""
                CREATE TABLE IF NOT EXISTS guild_config (
                    ...
                    staff_role       INTEGER,
                    welcome_message  TEXT DEFAULT 'Bienvenue {user_mention} !',
            ...
            """)
        logger.info(f"DB initialisee : {self.db_path}")
    ...
    def upsert_guild_config(self, guild_id: int, **kwargs: Any) -> None:
        ...
        if kwargs:
            ...
            self.execute(
                f"UPDATE guild_config SET {sets} WHERE guild_id = ?", tuple(values)
            )
